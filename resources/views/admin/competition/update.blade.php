                                ...
                                <div class="pt-2">
                                  <div class="grid grid-cols-6 gap-6">
                                    <div class="col-span-6 sm:col-span-4 mt-4">
                                      <form method="POST" enctype="multipart/form-data" action="{{ route('admin.update_competition_act', $data->id) }}">
                                        @csrf
                                        <div>
                                            <x-jet-label for="name" value="Nama Kompetisi" />
                                            <x-jet-input id="name" class="block mt-1 w-full" type="text" name="name" :value="$data->name" required autofocus autocomplete="name" placeholder="Nama kompetisi" />
                                            @error('name') <label class="text-red-700">{{ $message }}</label> @enderror
                                        </div>
                                        <div class="mt-4">
                                            <x-jet-label for="price" value="Harga pendaftaran" />
                                            <x-jet-input id="price" class="block mt-1 w-full" type="text" name="price" :value="$data->price" required autocomplete="price" placeholder="Harga pendaftaran..." />
